...
// NOTE:THE PRESTOAPI API HAS AN INBUILT CONNECTION TO DATABASE-i.e backend 

async function getRentals() {
  const rentalFetch = await fetch("https://rentals.prestoapi.com/api/rentals?limit=10");
  ...
  rentalJson.forEach((rental) => {
    ...
    const stars = rental.review_scores.review_scores_value ? parseInt(rental.review_scores.review_scores_value) / 2 : 0;
